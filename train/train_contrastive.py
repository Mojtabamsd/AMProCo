...
def train_contrastive(config_path, input_path, output_path, torch_profile):
    ...
    if config.training_contrastive.dataset == 'uvp':
        if world_size > 1:
            mp.spawn(train_uvp, args=(world_size, config, console, torch_profile), nprocs=world_size, join=True)
        else:
            train_uvp(config.base.gpu_index, world_size, config, console, torch_profile)

    elif config.training_contrastive.dataset == 'cifar100':
        if world_size > 1:
            mp.spawn(train_cifar, args=(world_size, config, console, torch_profile), nprocs=world_size, join=True)
        else:
            train_cifar(config.base.gpu_index, world_size, config, console, torch_profile)
...
def train_uvp(rank, world_size, config, console, torch_profile):
...
def train_cifar(rank, world_size, config, console, torch_profile):
...
def train(epoch, train_loader, model, criterion_ce, criterion_scl, optimizer, config, console, torch_profile):
    model.train()
    ...
    prof_result = None
    prof_ctx = make_profiler(torch_profile)
...
def validate(train_loader, val_loader, model, criterion_ce, config, console):
    ...
    with torch.no_grad():
        ...
        all_features = np.concatenate(all_features.cpu().numpy(), axis=0)
...
def make_profiler(torch_profile):
    if not torch_profile:

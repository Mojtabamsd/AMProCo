...
class BalSCL(nn.Module):
    ...
    def _compute_batch_centers(self, features, targets):
        ...
        return torch.stack(centers, dim=0)  # [C, D]

    def forward(self, features, targets):
        """
        features: [2B, D]   (two views stacked before calling)
        targets:  [B]       (labels for one view only)
        """
        device = self.device
        B, D = features.shape
        num_classes = len(self.cls_num_list)

        assert B % 2 == 0, f"Expected even number of features (2B), got {B}"
        B = B // 2  # recover original batch size

        # Duplicate labels for both views → [2B]
        targets_dup = torch.cat([targets, targets], dim=0)

        # Compute class centers from current batch → [C, D]
        centers1 = self._compute_batch_centers(features, targets_dup)

        # Concatenate features + centers → [2B + C, D]
        all_feats = torch.cat([features, centers1], dim=0)

        # Contrastive logits → [2B, 2B + C]
        logits = features.mm(all_feats.T) / self.temperature
        ...
        all_targets = torch.cat(
            [targets_dup, torch.arange(num_classes, device=device)], dim=0
        )
        # Positive pairs mask → [2B, 2B + C]
        mask = torch.eq(targets_dup.unsqueeze(1), all_targets.unsqueeze(0)).float()

        # Mask out self-contrast for the 2B samples
        logits_mask = torch.ones_like(mask)
        logits_mask[:, :2*B].fill_diagonal_(0)  # remove self-pairs
        mask = mask * logits_mask

        # ---- Compute log-probs ----
        exp_logits = torch.exp(logits) * logits_mask  # [2B, 2B + C]
